    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if(saveSettingsBtn) {
            saveSettingsBtn.addEventListener('click', function() {
                ...
                window.PomodoroManager.reset(mins * 60);
            });
        }
        ...
        if(stopBtn) stopBtn.addEventListener("click", () => window.PomodoroManager.pause());
        ...
        window.onPomodoroTick = function(remaining, totalTime, isRunning) {
            ...
            if(progressCircle) {
                ...
                let safeTotal = totalTime > 0 ? totalTime : 1500;
        ...
        if(window.PomodoroManager) window.PomodoroManager.tick();
    });
</script>'''

with open(filepath, "w", encoding="utf-8") as f:
    f.write(content)
print("Updated timer.html successfully.")
